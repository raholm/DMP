...
def train_models():
	train_experiment(get_qlearning_experiment(), get_states(), get_rewards())


def experiment01_qlearning():
	experiment = get_qlearning_experiment()
	...
	print("Average Correlation between Reward and Game Score: %0.03f, std: %0.03f" % (np.mean(corr_coefs), np.std(corr_coefs)))
	print("Average Abs. Correlation between Reward and Game Score: %0.03f, std: %0.03f" % (np.mean(np.abs(corr_coefs)), np.std(np.abs(corr_coefs))))
