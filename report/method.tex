...
\begin{document}
    ...
    %    The other family is characterized by
    %
    %    \begin{center}
    %        \begin{tabular}{| l |}
    %            \hline
    %            \textbf{Reward at the end of the episode} \\ \hline
    %            $+ 1000\ *$ length of snake \\
    %            $- 100 * (\text{\# of time steps})^2$ \\
    %            $- 10000\ *$ reason for death, either self-collision or with the game board \\ \hline
    %            \textbf{Reward after each time step} \\ \hline
    %            $-$ the manhattan distance from snake head to food source \\ \hline
    %        \end{tabular}
    %    \end{center}
    %
    %    and we choose to create 4 reward functions based on these characteristics. All have the reward based on the length of the snake and then either no punishment at the end of the episode or one of the different punishments.
